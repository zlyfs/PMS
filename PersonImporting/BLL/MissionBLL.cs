...
namespace PersonImporting.BLL
{
    public class MissionBLL
    {
        ...
        public bool CheckMissionExist(string missionName)
        {
            //根据群组名称获取群组集合
            var missionList = missionBLL.GetListBy(g => g.SMSMissionName == missionName).ToList();
            //判断集合是否为空
        ...
        public bool CheckMissionExist(int SMID)
        {
            //根据群组名称获取群组集合
            var missionList = missionBLL.GetListBy(g => g.SMID == SMID).ToList();
            //判断集合是否为空
            if (missionList.Count() == 0)
            {
                return false;
        ...
        public string getMissionName(int SMID)
        {
            string missionName = "";
            if(CheckMissionExist(SMID))
            {
                missionName = missionBLL.GetListBy(g => g.SMID == SMID).FirstOrDefault().SMSMissionName;
            }
            return missionName;
        }
    }
